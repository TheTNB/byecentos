...
get_release_file_url() {
    ...
    local -r arch="${2}"
    echo "${ALMA_RELEASE_URL:-https://mirrors.aliyun.com/almalinux/almalinux-release-latest-${os_version}.${arch}.rpm}"
}
...
get_repos_file_url() {
    local -r os_version="${1:0:1}"
    local -r arch="${2}"
    echo "${ALMA_REPOS_URL:-https://mirrors.aliyun.com/almalinux/almalinux-repos-latest-${os_version}.${arch}.rpm}"
}
...
install_rpm_pubkey() {
    ...
    local -r tmp_dir="${1}"
    local -r os_version="${2:0:1}"
    local -r ALMA_PUBKEY_URL="https://mirrors.aliyun.com/almalinux/RPM-GPG-KEY-AlmaLinux-${os_version}"
    local -r pubkey_url="${ALMA_PUBKEY_URL:-https://mirrors.aliyun.com/almalinux/RPM-GPG-KEY-AlmaLinux}"
